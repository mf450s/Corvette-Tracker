...
from .storage import EDITABLE_FIELDS, PROTECTED_OVERRIDE_FIELDS
...
<head>
  ...
  <style>
    ...
    * {{ box-sizing:border-box; }} body {{ margin:0; font-family:Inter,ui-sans-serif,system-ui,sans-serif; background:var(--bg); color:var(--text); }}
    ...
    .image {{ aspect-ratio:16/9; background:#18181b; display:block; position:relative; overflow:hidden; }} .image img {{ width:100%; height:100%; object-fit:cover; }} .score-badge {{ position:absolute; top:10px; left:10px; z-index:1; display:inline-flex; align-items:center; justify-content:center; min-width:54px; padding:7px 11px; border-radius:999px; background:linear-gradient(135deg,#ef4444,#f59e0b); color:white; font-weight:900; box-shadow:0 10px 26px rgba(0,0,0,.38); }} .speedo-badge {{ position:absolute; top:10px; right:10px; z-index:1; display:inline-flex; align-items:center; justify-content:center; padding:7px 11px; border-radius:999px; background:rgba(24,24,27,.9); color:#fbbf24; font-size:12px; font-weight:800; box-shadow:0 10px 26px rgba(0,0,0,.38); }} .body {{ padding:16px; }}
