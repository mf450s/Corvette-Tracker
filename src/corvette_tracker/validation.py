...
def _flag(*, field: str, severity: str, code: str, message: str) -> dict[str, Any]:
    return {
        "field": field,
        "severity": severity,
        "code": code,
...
def _check_price(listing: Listing) -> list[dict[str, Any]]:
...
def _check_mileage(listing: Listing) -> list[dict[str, Any]]:
    flags: list[dict[str, Any]] = []
...
def _check_power(listing: Listing) -> list[dict[str, Any]]:
    flags: list[dict[str, Any]] = []
    hp = listing.power_hp
    if hp is not None:
        if hp < 0:
...
def _check_year(listing: Listing) -> list[dict[str, Any]]:
    flags: list[dict[str, Any]] = []
...
def _check_missing_trim(listing: Listing) -> list[dict[str, Any]]:
    flags: list[dict[str, Any]] = []
...
def _check_body_style(listing: Listing) -> list[dict[str, Any]]:
    flags: list[dict[str, Any]] = []
...
def _check_missing_price_and_label(listing: Listing) -> list[dict[str, Any]]:
...
def _check_engine_trim_consistency(listing: Listing) -> list[dict[str, Any]]:
    flags: list[dict[str, Any]] = []
        ...
        if listing.price_eur < lower:
            flags.append(
                _flag(
                    field="price_eur",
                    severity=INFO,
                    code="price_below_historical_range",
            ...
            flags.append(
                _flag(
                    field="price_eur",
                    severity=INFO,
                    code="price_above_historical_range",
        ...
        if listing.mileage_km > upper:
            flags.append(
                _flag(
                    field="mileage_km",
                    severity=INFO,
                    code="mileage_above_historical_range",
...
def validate_listing(listing: Listing) -> list[dict[str, Any]]:
...
def apply_validation_flags(listing: Listing, flags: list[dict[str, Any]] | None = None) -> Listing:
...
def safe_normalize_listing(
    ...
    **kwargs: Any,
    ...
    """
    try:
        result = normalize_func(*args, **kwargs)
        return cast(Listing | None, result)
    except Exception as exc:
        safe_normalize_listing._last_error[0] = exc  # ty: ignore[unresolved-attribute] - dynamic compatibility attribute
        return None


safe_normalize_listing._last_error = [None]  # ty: ignore[unresolved-attribute] - dynamic compatibility attribute
