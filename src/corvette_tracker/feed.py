...
<head>
  ...
  <style>
    ...
    * {{ box-sizing: border-box; }} body {{ margin:0; font-family: Inter, ui-sans-serif, system-ui, -apple-system, Segoe UI, sans-serif; background:radial-gradient(circle at top left, rgba(239,68,68,.22), transparent 32rem), linear-gradient(180deg,#0b0d12,var(--bg)); color:var(--text); }}
    ...
    .card {{ overflow:hidden; border-radius:26px; }} .image {{ display:block; aspect-ratio:16/10; background:#12151b; overflow:hidden; }} .image img {{ width:100%; height:100%; object-fit:cover; transition:transform .25s ease; }} .card:hover img {{ transform:scale(1.035); }} .placeholder {{ height:100%; display:grid; place-items:center; color:var(--muted); }}
